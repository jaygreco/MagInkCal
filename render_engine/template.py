...
def __c(tag):
    ...
    empty_elements = ["area", "base", "br", "col", "embed", "hr", "img", "input", "link", "meta", "source", "track", "wbr"]
    return "" if tag in empty_elements else f"</{tag}>"

# Only a small subset of these are needed right now
...
def template(tag, c="", id="", ext_attr={}, body=('')):
    ...
    inner += ' '.join([f"{k}='{v}'" for k,v in ext_attr.items()]) if ext_attr else ""
...
if __name__ == "__main__":
    ...
    l = []
    for d in range(0, 7):
        l.append(template('li', c='font-weight-bold text-uppercase', body=days[(d + start_day) % 7]))
    test1 = '\n'.join(l)
    print(test1)
